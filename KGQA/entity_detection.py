...
class EntityDetection(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        target_size = config.label  # 4  ['<unk>' '<pad>' 'O' 'I']
        ...
        if config.entity_detection_mode.upper() == 'LSTM':
            self.lstm = nn.LSTM(input_size=config.words_dim,  # 300
                               hidden_size=config.hidden_size,
                               num_layers=config.num_layer,
                               dropout=config.rnn_dropout,
                               bidirectional=True)
        elif config.entity_detection_mode.upper() == 'GRU':
            self.gru = nn.GRU(input_size=config.words_dim,  # 300
                                hidden_size=config.hidden_size,
                                num_layers=config.num_layer,
                                dropout=config.rnn_dropout,
                                bidirectional=True)
    ...
    def forward(self, x):
        ...
        scores = F.log_softmax(tags, dim=1)  # 0是对列做归一化，1是对行做归一化
        return scores
