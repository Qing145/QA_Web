...
if args.cuda:
    ...
    dev_iter = data.Iterator(polyu_dataset_dev, batch_size=args.batch_size, device=torch.device('cuda', args.gpu), train=False,
                             repeat=False, sort=False, shuffle=False, sort_within_batch=False)
else:
    ...
    #                          sort_within_batch=False)
    dev_iter = data.Iterator(polyu_dataset_dev, batch_size=args.batch_size, train=False, repeat=False, sort=False, shuffle=False,
                             sort_within_batch=False)
...
index2tag = np.array(ED.vocab.itos)  # ['<unk>' '<pad>' 'O' 'I']  <UNK>: 低频词或未在词表中的词  <PAD>: 补全字符
...
while True:
    ...
    for batch_idx, batch in enumerate(train_iter):
        ...
        # if torch.max(scores, 1)[1].size() == 192 and batch.ed.size() == [13, 16]:
        #     print(batch.ed.data)
        ...
        if iterations % args.dev_every == 0:
            ...
            if R > best_dev_R:
                ...
                if iters_not_improved > patience:  # # for early stopping
                    early_stop = True
                    break
